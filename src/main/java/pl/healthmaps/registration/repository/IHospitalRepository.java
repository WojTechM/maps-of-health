...
public interface IHospitalRepository extends JpaRepository<Hospital, Long> {
    ...
    @Query(value = "select * from hospital as h left join location as l on (h.id = l.hospital_id) where l.latitude between (?1 -5) and (?1 + 5) and l.longitude between (?2 - 5) and (?2 + 5);", nativeQuery = true)
